...
if ($_POST['action'] == "postdata")
{
	...
	{
		$messagealert .= '<dt>Line no: '.$_POST['currentcount'].'</dt>
          <dd>- Agreement no: '.$agreement_number.'</dd>
          <dd>- Contact ID: '.$contact_Id.'</dd>
          '.validateTaxRate().validatePaymentChannel().validateAccount($account, $category).validateContactId($contact_Id).validateCategory($category).'<hr>';
         echo $messagealert;
		exit();
	}
