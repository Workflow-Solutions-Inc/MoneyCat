...
$excel = PHPExcel_IOFactory::load("Quarterly Summary List of Purchases w Excel, PDF, CSV and DAT file.xlsx");
    ...
    { 
    	...
    	$query ="select monthname(invoicedate) as invdate, ci.taxtype, ci.customerName,
				...
				ifnull((select sum(lineamount) from invoicetrans where Invoiceid = iv.id and taxtype != 'NONE' and taxamount = 0),0 ) as 'zero',
				...
				where invoicetype = 'ACCREC' and year(iv.invoicedate) = '$year' and month(iv.invoicedate) between $quart and $quart+2;";
